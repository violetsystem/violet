...
static const struct clust_size_s cluster_size_lut[] = {
	{      66600,	0  },			// partitions up to 32.5 MB	
	{     532480,	1  },			// partitions up to 260 MB	, 0.5k clusters
	{   16777216,	8  },			// partitions up to 8 GB		, 4k clusters
	{   33554432,	16 },			// partitions up to 16 GB	, 8k clusters
	{   67108864,	32 },			// partitions up to 32 GB	, 16k clusters
	{ 0xFFFFFFFF,	64 }			// partitions > 32 GB		, 32k clusters<
};
...
static fstatus fat_follow_path(struct volume_s* vol, struct dir_s* dir, const char* path, u32 length);
...
static u8 fat_read(struct volume_s* vol, u32 lba) {
	...
	if (vol->buffer_lba != lba) {
		...
		if (!partition_read(vol->partition, vol->buffer, lba, 1)) {
			return 0;
		}
		vol->buffer_lba = lba;
	}
...
static u8 fat_flush(struct volume_s* vol) {
	if (vol->buffer_dirty) {
		if (!partition_write(vol->partition, vol->buffer, vol->buffer_lba, 1)) {
			return 0;
		}
		vol->buffer_dirty = 0;
	}
...
/// Mounts a physical partition. It checks for a valid FAT32 file system in all
/// available partition partitions. All valid file system is dynamically allocated
/// and added to the system volumes
/// 
/// Note that this is the only functions referencing the `partition` parameter. All
/// further interactions happend will via the volume letter e.g. D: drive.
u8 partition_mount(partition_t* partition) {	
	// Search for a valid FAT32 file systems on all valid paritions
	if (!partition_read(partition, mount_buffer, 0, 1)) {
		return 0;
	}
	...
	if (fat_search(mount_buffer)) {
		...
		vol->fsinfo_lba = fat_load16(mount_buffer +
			BPB_32_FSINFO);
		vol->fat_lba = fat_load16(mount_buffer + 
			BPB_RSVD_CNT);
...
/// Remove the volumes corresponding with a physical partition and delete the memory.
/// This function must be called before a storage device is unplugged, if not,
/// cached data may be lost. 
u8 partition_eject(partition_t* partition) {
	free(partition->volume);
	return 1;
}
...
/// directory
fstatus fat_dir_open(struct volume_s* vol, struct dir_s* dir, const char* path, u16 length) {
	return fat_follow_path(vol, dir, path, length);
}
...
fstatus fat_dir_make(struct volume_s* vol, const char* path) {
	return FSTATUS_OK;
}
...
fstatus fat_file_open(struct volume_s* vol, struct file_s* file, const char* path, u16 length) {
	// Make a pointer to the directory where the file is stored
	struct dir_s dir;
	fstatus status = fat_follow_path(vol, &dir, path, length);
